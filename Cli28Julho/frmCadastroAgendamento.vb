...
Public Class frmCadastroAgendamento
  ...
  Const const_GridHorario_DataNascimento As Integer = 9
  Const const_GridHorario_IC_TIPO_RETORNO As Integer = 10
  ...
  Private Sub FormatarTela()
    ...
    objGrid_Coluna_Add(grdHorario, "Dt. Nascimento", 100)
  ...
  Private Sub cmdGravar_Click(sender As Object, e As EventArgs) Handles cmdGravar.Click
    ...
    sSqlText = "SELECT COUNT(*)" &
               ...
               " WHERE AGEND.ID_PESSOA = " & psqPessoa.ID_Pessoa &
                 " AND AGEND.ID_PESSOA_PROFISSIONAL = " & cboProfissional.SelectedValue &
                 " AND AGEND.ID_PROCEDIMENTO = " & psqProcedimento.Identificador &
                 " AND GPPCD.TP_BLOQUEAR_AGENDAMENTO_DUPLICADO = 'S'" &
                 " AND AGEND.ID_OPT_STATUS IN (" & enOpcoes.StatusAgendamento_Agendado.GetHashCode() & "," &
                 ...
                 " AND CAST(AGEND.DH_AGENDAMENTO AS DATE) = " & FNC_QuotedStr(txtDataAgendamento.DateTime.Date) &
                 " AND AGEND.SQ_AGENDAMENTO <> " & iSQ_AGENDAMENTO
    If DBQuery_ValorUnicoInt(sSqlText) > 0 Then
      FNC_Mensagem("já foi relaciondo esse agendamento para a data de hoje")
      Exit Sub
    End If

    FNC_QuotedStr(txtDataAgendamento.DateTime.Date)
    ...
    If Not chkAtualizarListagemAutomaticamente.Checked Then
      ...
      With grdHorario.Rows(iRowIndex)
        ...
        .Cells(const_GridHorario_DataNascimento).Value = psqPessoa.dDT_NASC_ABERTURA.ToString().Substring(0, 10)
  ...
  Private Sub Calendario_LimparLinha(iLinha As Integer)
    If iLinha >= 0 Then
      ...
      grdHorario.Rows(iLinha).Cells(const_GridHorario_DataNascimento).Value = ""
      ...
      For iCont_01 = 0 To oData.Rows.Count - 1
          ...
          With oData.Rows(iCont_01)
            ...
            If grdHorario.Rows(iCont_02).Cells(const_GridHorario_HORA).Value = sHora Then
              ...
              If FNC_NVL(grdHorario.Rows(iCont_02).Cells(const_GridHorario_SQ_AGENDAMENTO).Value, 0) <> -1 Then
                ...
                If Not objDataTable_CampoVazio(.Item("DT_NASC_ABERTURA")) Then
                  grdHorario.Rows(iCont_02).Cells(const_GridHorario_DataNascimento).Value = .Item("DT_NASC_ABERTURA").ToString().Substring(0, 10)
                End If
        ...
        If Not bAchou Then
          ...
          With oData.Rows(iCont_01)
            ...
            grdHorario.Rows(oRow.Index).Cells(const_GridHorario_DataNascimento).Value = .Item("DT_NASC_ABERTURA").ToString().Substring(0, 10)
  ...
  Private Sub psqPessoa_Adicionado() Handles psqPessoa.Adicionado
    If iLinhaSelecionada > -1 Then
      With grdHorario.Rows(iLinhaSelecionada)
        .Cells(const_GridHorario_ID_PACIENTE).Value = psqPessoa.ID_Pessoa
        .Cells(const_GridHorario_Paciente).Value = psqPessoa.DS_Pessoa
        If psqPessoa.cboPessoa.SelectedItem IsNot Nothing Then
          .Cells(const_GridHorario_Paciente).Value = psqPessoa.cboPessoa.SelectedItem(const_GridListagemPessoa_DtNascimentoAbertura)
        End If
  ...
  Private Sub CarregarInformacoes(iID_PACIENTE As Integer)
    ...
    sSqlText = "SELECT TOP 3 CD_AGENDAMENTO, DH_AGENDAMENTO" &
               ...
               " WHERE ID_PESSOA = " + iID_PACIENTE.ToString() &
                 ...
                 " AND ID_OPT_STATUS IN (" & enOpcoes.StatusAgendamento_Agendado.GetHashCode() & "," &
                                              enOpcoes.StatusAgendamento_AguardandoPagamento.GetHashCode() & "," &
                                              enOpcoes.StatusAgendamento_AguardandoAtendimento.GetHashCode() & "," &
                                              enOpcoes.StatusAgendamento_Atendido.GetHashCode() & ")" &
