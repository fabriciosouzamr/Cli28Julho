...
Module modFuncao
  ...
  Public Const const_ConfiguracaoTela_MouseOut As String = "MO"
  Public Const const_ConfiguracaoTela_MouseIn As String = "MI"
  Public Const const_ConfiguracaoTela_MouseClick As String = "MC"
  Public Const const_ConfiguracaoTela_Desabilitar As String = "DS"
  ...
  Public Function FNC_ConfiguracaoTela(iTela As Integer) As String
    Dim sDS_PATH_IMAGEM_FUNDO As String

    sDS_PATH_IMAGEM_FUNDO = DBQuery_ValorUnico("SELECT DS_PATH_IMAGEM_FUNDO FROM TB_CONFIG_TELA WHERE ID_TELA = " & iTela.ToString())

    Return sDS_PATH_IMAGEM_FUNDO
  End Function

  Public Function FNC_Impressora_Padrao() As String
    Dim printer As New System.Drawing.Printing.PrinterSettings
    Dim DefaultPrinterName As String

    Try
      DefaultPrinterName = printer.PrinterName
  ...
  Public Function FNC_ConfiguracaoTela_Botao(iTela As Integer, sStatus As String) As String
    Dim sDS_PATH_IMAGEM_FUNDO As String

    Try
      sDS_PATH_IMAGEM_FUNDO = DBQuery_ValorUnico("SELECT DS_PATH_IMAGEM_FUNDO FROM TB_CONFIG_TELA WHERE ID_TELA = " & iTela.ToString())
      sDS_PATH_IMAGEM_FUNDO = sDS_PATH_IMAGEM_FUNDO.Replace("##", sStatus)
    Catch ex As Exception
      sDS_PATH_IMAGEM_FUNDO = ""
    End Try

    Return sDS_PATH_IMAGEM_FUNDO
  End Function

  Public Function FNC_Pessoa_Horario_Bloqueio_Data(iProfissional As Integer, dData As Date) As Boolean
    Dim sSqlText As String
    Dim bRet As Boolean = False

    Try
      sSqlText = "SELECT COUNT(*) FROM VW_PESSOA_HORARIO_BLOQUEIO" +
                 " WHERE ID_PESSOA = " + iProfissional.ToString() +
                   " AND DT_BLOQUEIO_INCIO <= " + FNC_QuotedStr(FNC_DBDataToString(dData, True)) +
                   " AND DT_BLOQUEIO_FIM >= " + FNC_QuotedStr(FNC_DBDataToString(dData, True))

      bRet = (DBQuery_ValorUnico(sSqlText) > 0)
    Catch ex As Exception
    End Try
  ...
  Public Function FNC_TextoPrimeiraMaiuscula(sTexto As String) As String
    Dim iCont As Integer

    sTexto = sTexto.ToLower()

    For iCont = 0 To sTexto.Length - 1
      If iCont = 0 Then
        sTexto = sTexto.Substring(iCont, 1).ToUpper() + sTexto.Substring(iCont + 1)
      Else
        If Trim(sTexto.Substring(iCont - 1, 1)) = "" Then
          sTexto = sTexto.Substring(0, iCont) + sTexto.Substring(iCont, 1).ToUpper() + sTexto.Substring(iCont + 1)
        End If
      End If
    Next

    sTexto = sTexto.Replace(" Da ", " da ").Replace(" De ", " de ").Replace(" Di ", " di ").Replace(" Do ", " do ")
  ...
  Public Function FNC_DBDataToString(oData As Object, Optional bComHora As Boolean = False) As String
    ...
    If IsDate(oData) Then
      ...
      If bComHora Then
        sAux = sAux + " " + FNC_StrZero(oData.Hour, 2) + ":" + FNC_StrZero(oData.Minute, 2) + ":" + FNC_StrZero(oData.Second, 2)
      End If
    End If
  ...
  Public Function FNC_URL_Executar(sURL) As Boolean
    Try
      Dim client As WebClient = New WebClient()
      Dim content As LPWSTR = client.DownloadString(sURL)
    Catch ex As Exception
...
Sair:
  ...
  Public Function FNC_MaiorNumero(ParamArray oNumero() As Double) As Double
    Dim Numero As Double = 0
    Dim iCont As Integer

    For iCont = LBound(oNumero) To UBound(oNumero)
      If oNumero(iCont) > Numero Then
        Numero = oNumero(iCont)
      End If
    Next
  ...
  Public Function FNC_Porcentagem(dValor As Double,
                                  dFator As Double,
                                  Optional iCasasDecimal As Integer = 2,
                                  Optional bAdicionarAoPrincial As Boolean = False) As Double
    If dFator = 0 Or dValor = 0 Then
      Return 0
    Else
      Return Math.Round((dValor * dFator / 100) + IIf(bAdicionarAoPrincial, dValor, 0), iCasasDecimal)
    End If
  End Function
  ...
  Public Sub FNC_AbriTela(ByRef oForm As Form,
                          ...
                          Optional Parent As Boolean = True)
    ...
    If Not oFormMDI Is Nothing And Not bModal And Parent Then
  ...
  Public Function FNC_Collection_Existe(ByVal Variavel As Collection,
                                        ByVal Valor As Object) As Boolean
    Dim iCont As Integer
    Dim achou As Boolean = False

    If Not Variavel Is Nothing Then
  ...
  Public Function FNC_Lista_Para_Collection(sLista As String,
                                            Optional ByVal Separador As String = ",") As Collection
    Dim oRet As New Collection

    If sLista.IndexOf(Separador) > 0 Then
      For Each sItem As String In sLista.Split(Separador)
        oRet.Add(Trim(sItem))
      Next
    Else
      oRet.Add(sLista)
