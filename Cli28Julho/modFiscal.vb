...
Sair:
  ...
  Private Function FNC_Fiscal_Configuracao(Optional eModeloDocumento As ModeloDocumento = ModeloDocumento.NFe) As NFe.Utils.ConfiguracaoServico
    ...
    Dim oEstado As DFe.Classes.Entidades.Estado
...
Sair:
  ...
  Public Function FNC_Fiscal_DocumentoFiscal_Transmitir(iSQ_DOCUMENTOFISCAL As Integer,
    ...
    Dim oNFe_Servico_RetornoNFeRetAutorizacao As NFe.Servicos.Retorno.RetornoNFeRetAutorizacao
    Dim oNFe_protNFe As NFe.Classes.Protocolo.protNFe
    Dim oNFe_Proc As NFe.Classes.nfeProc
    Dim oConfig As NFe.Utils.ConfiguracaoServico
    Dim sMens As String
    ...
    If FNC_Fiscal_DocumentoFiscal_Transmitir_Validar(iSQ_DOCUMENTOFISCAL) Then
      Try
        ...
        If Not oNFe Is Nothing Then
          ...
          If Not bVerificarProtocolo Then
            ...
            If FNC_Fiscal_NFe_StatusProcessamento(oNFe_Servico_RetornoNFeAutorizacao.Retorno.cStat) = enOpcoes_NFe_StatusProcessamento.LoteRecebidoComSucesso Then
              oNFe_Servico_RetornoNFeRetAutorizacao = oNFe_Servico.NFeRetAutorizacao(oNFe_Servico_RetornoNFeAutorizacao.Retorno.infRec.nRec)
              FNC_Fiscal_Historico(FNC_Fiscal_NFe_StatusProcessamento(oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.cStat), iSQ_DOCUMENTOFISCAL, "Retorno de autorização - " & oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.xMotivo & " - Recibo: " & oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.nRec)

              If oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.protNFe.Count > 0 Then
                oNFe_protNFe = oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.protNFe(0)

                sMens = "Chave NFe: " & oNFe_protNFe.infProt.chNFe & vbCrLf &
        "Protocolo nº: " & oNFe_protNFe.infProt.nProt & vbCrLf &
        "Motivo: " & oNFe_protNFe.infProt.xMotivo

                FNC_Fiscal_Historico(FNC_Fiscal_NFe_StatusProcessamento(oNFe_protNFe.infProt.cStat), iSQ_DOCUMENTOFISCAL, "Retorno de autorização - " & oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.xMotivo & vbCrLf & sMens)

                bProblemaNFe = bProblemaNFe Or (FNC_Fiscal_NFe_StatusProcessamento(oNFe_protNFe.infProt.cStat) <> enOpcoes_NFe_StatusProcessamento.AutorizadoUsoNFe)

                oNFe_protNFe = Nothing
              End If

              System.Windows.Forms.Application.DoEvents()
              System.Windows.Forms.Application.DoEvents()

              bVerificarProtocolo = (FNC_Fiscal_NFe_StatusProcessamento(oNFe_Servico_RetornoNFeRetAutorizacao.Retorno.cStat) = enOpcoes_NFe_StatusProcessamento.LotePocessado And
                       Not bProblemaNFe)
            End If
          End If
      ...
      Catch ex As Exception
        ...
        oNFe_Servico_RetornoNFeRetAutorizacao = Nothing
...
Sair:
  ...
  Private Function FNC_Fiscal_DocumentoFiscal_FormaPagamento(iID_FORMAPAGAMENTO As Integer) As NFe.Classes.Informacoes.Pagamento.FormaPagamento
    Dim eFormaPagamento As NFe.Classes.Informacoes.Pagamento.FormaPagamento = FormaPagamento.fpOutro

    Select Case iID_FORMAPAGAMENTO
        ...
        With oData_01.Rows(0)
          ...
          Else
            ...
            Else
              ...
              If FNC_NVL(.Item("CD_OPT_PJ_CONTRIBUICAO_ICMS"), 0) = NFe.Classes.Informacoes.Destinatario.indIEDest.NaoContribuinte.GetHashCode() Then
                oNFe.infNFe.ide.indFinal = NFe.Classes.Informacoes.Identificacao.Tipos.ConsumidorFinal.cfConsumidorFinal
              Else
...
NFe.Classes.Informacoes.Destinatario.indIEDest.ContribuinteICMS Then
 ...
 (oNFe.infNFe.ide.mod = ModeloDocumento.NFe And .Item("CD_SERVICO_VERSAO") = const_NFe_VersaoServico_ve400) Then
            sSqlText = "SELECT PEDPG.ID_FORMAPAGAMENTO, PEDPG.VL_PAGAMENTO, ADMFN.CD_CPF_CNPJ" &
                       " FROM TB_DOCUMENTOFISCAL DCFSC" &
                        " INNER JOIN TB_PEDIDO_PAGAMENTO PEDPG ON PEDPG.SQ_PEDIDO_PAGAMENTO = DCFSC.ID_PEDIDO_PAGAMENTO" &
                        " INNER JOIN TB_FORMAPAGAMENTO FPGTO ON FPGTO.SQ_FORMAPAGAMENTO = PEDPG.ID_FORMAPAGAMENTO" &
                       " LEFT JOIN TB_PESSOA ADMFN ON ADMFN.SQ_PESSOA = FPGTO.ID_ADMINISTRADORAFINANCEIRA" +
            ...
            Else
              For iCont = 0 To oData_02.Rows.Count - 1
                ...
                Else
                  ...
                  If Not FNC_CampoNulo(oData_02.Rows(iCont).Item("CD_CPF_CNPJ")) Then
