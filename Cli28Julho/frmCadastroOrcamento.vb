...
Public Class frmCadastroOrcamento
  ...
  Const const_GrdProcedimento_SolicitanteJaDefinido = 10
  Const const_GrdProcedimento_ID_PESSOA_PROFISSIONAL_SOLICITANTE = 11
  Const const_GrdProcedimento_BNT_Solicitante = 12
  ...
  Dim optStatus = enOpcoes.Processo_Acao_Inclusao
  ...
  Private Sub frmCadastroOrcamento_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    objGrid_Coluna_Add(grdProcedimento, "SolicitanteJaDefinido", 0)
    objGrid_Coluna_Add(grdProcedimento, "ID_PESSOA_PROFISSIONAL_SOLICITANTE", 0)
    objGrid_Coluna_Add(grdProcedimento, "Solicitante", 50, , , ColumnStyle.Button)
    ...
    If iSQ_ORCAMENTO_CLIENTE > 0 Then
      optStatus = enOpcoes.Processo_Acao_Alteracao
      ...
      sSqlText = "SELECT OCPRC.SQ_ORCAMENTO_CLIENTE_PROCEDIMENTO," &
                        ...
                        "OCPRC.VL_ORCADO," &
                        "OCPRC.ID_CONVENIO," &
                        "OCPRC.ID_CLINICA_PROCEDIMENTO," &
                        "PESSO_SOLIC.NO_PESSOA," &
                        "IIF(CLPCD.ID_CLINICA_ATENDIMENTO IS NULL, 'N', 'S')," &
                        "OCPRC.ID_PESSOA_PROFISSIONAL_SOLICITANTE" &
                 " FROM TB_ORCAMENTO_CLIENTE_PROCEDIMENTO OCPRC" &
                  ...
                  " INNER JOIN TB_PESSOA PESSO ON PESSO.SQ_PESSOA = OCPRC.ID_PESSOA_PROFISSIONAL" &
                   " LEFT JOIN TB_CLINICA_PROCEDIMENTO CLPCD ON CLPCD.SQ_CLINICA_PROCEDIMENTO = OCPRC.ID_CLINICA_PROCEDIMENTO" &
                   " LEFT JOIN TB_CLINICA_ATENDIMENTO CLATD ON CLATD.SQ_CLINICA_ATENDIMENTO = CLPCD.ID_CLINICA_ATENDIMENTO" &
                   " LEFT JOIN TB_PESSOA PESSO_SOLIC ON PESSO_SOLIC.SQ_PESSOA = CLATD.ID_PESSOA_PROFISSIONAL" &
      ...
      objGrid_Carregar(grdProcedimento, sSqlText, New Integer() {const_GrdProcedimento_SQ_ORCAMENTO_CLIENTE_PROCEDIMENTO,
                                                                 ...
                                                                 const_GrdProcedimento_SolicitanteJaDefinido,
                                                                 const_GrdProcedimento_ID_PESSOA_PROFISSIONAL_SOLICITANTE})
  ...
  Private Sub cmdGravar_Click(sender As Object, e As EventArgs) Handles cmdGravar.Click
    ...
    If DBExecutar(sSqlText, DBParametro_Montar("SQ_ORCAMENTO_CLIENTE", iSQ_ORCAMENTO_CLIENTE,, ParameterDirection.InputOutput),
      ...
      For iCont = 0 To grdProcedimento.Rows.Count - 1
        With grdProcedimento.Rows(iCont)
          ...
          DBExecutar(sSqlText, DBParametro_Montar("SQ_ORCAMENTO_CLIENTE_PROCEDIMENTO", 0),
                               ...
                               DBParametro_Montar("ID_PESSOA_PROFISSIONAL_SOLICITANTE", FNC_NVL(.Cells(const_GrdProcedimento_ID_PESSOA_PROFISSIONAL_SOLICITANTE).Value, 0)),
      ...
      FNC_Historico_Incluir(enOpcoes.Processo_Historico_Agendamento_CadastroOrcamento, optStatus, 0, iSQ_ORCAMENTO_CLIENTE, "Pessoa: " + psqPessoa.cboPessoa.Text + Environment.NewLine +
                                                                                                                            "Convênio: " + cboConvenio.Text, txtCodigo.Text)
  ...
  Private Sub grdProcedimento_ClickCellButton(sender As Object, e As CellEventArgs) Handles grdProcedimento.ClickCellButton
    Select Case e.Cell.Column.Index
      Case const_GrdProcedimento_BNT_Solicitante
        If FNC_NVL(e.Cell.Row.Cells(const_GrdProcedimento_SolicitanteJaDefinido).Value, "N") = "S" Then
          FNC_Mensagem("Não é posível alterar o profissional solicitante")
        Else
          ...
          If oForm.SQ_PESSOA > 0 Then
            e.Cell.Row.Cells(const_GrdProcedimento_Solicitante).Value = oForm.NO_PESSOA
            e.Cell.Row.Cells(const_GrdProcedimento_ID_PESSOA_PROFISSIONAL_SOLICITANTE).Value = oForm.SQ_PESSOA
          End If
